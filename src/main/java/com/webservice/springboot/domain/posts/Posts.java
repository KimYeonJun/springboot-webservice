...
public class Posts extends BaseTimeEntity {
    //Posts클래스를 Entity클래스라고 의미한다. DB의 테이블과 매칭된다.
    //Entity 클래스에서는 절대 Setter메소드를 만들지 않는다.
    @Id //해당 테이블의 PK 필드를 의미
    @GeneratedValue(strategy = GenerationType.IDENTITY) //PK의 생성 규칙 , GenerationType.IDENTITY를 추가해야 auto_increment가 된다.
    private Long id;

    @Column(length = 500, nullable = false) //테이블의 칼럼을 나타내며 굳이 선언하지 않더라도 해당 클래스의 필드는 모두 칼럼이 된다.
                                            //사용하는 이유는, 기본값 외에 추가로 변경이 필요한 옵션이 있으면 사용.
                                            //문자열의 경우 VARCHAR(255)가 기본값인데, 사이즈를 500으로 늘리거나, 타입을 TEXT로 변경하거나 등
